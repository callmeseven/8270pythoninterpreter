...
%{
	...
	#include "includes/functionTable.h"
...
%type<node> test star_COMMA_test COMMA testlist opt_arglist arglist opt_test arith_expr term factor power atom opt_yield_test pick_yield_expr_testlist
                        ...
                        return_stmt flow_stmt STRING DOUBLESTAR STAR EQUAL LPAR fplist RPAR NOT comparison expr xor_expr and_expr shift_expr small_stmt print_stmt stmt simple_stmt suite if_stmt

%type<id> pick_multop pick_PLUS_MINUS pick_unop augassign
%type<intNumber> comp_op trailer star_trailer
%type<vec> plus_stmt
...
funcdef // Used in: decorated, compound_stmt
	: DEF IDENT { TableManager::getInstance().pushScope();} parameters COLON suite { FunctionTable::getInstance().setSuite($2,$6); TableManager::getInstance().popScope();}  
	;
...
expr_stmt // Used in: small_stmt
	: testlist augassign pick_yield_expr_testlist { if($2 == "PLUSEQ"){
			Node* node = new AddBinaryNode($1,$3);
			$$ = new AsgBinaryNode($1, node);
			pool.add(node);
			pool.add($$);
			}
		if($2 == "SUBEQ"){
		...
		if($2 == "PEREQ"){
                        Node* node = new PerBinaryNode($1,$3);
                        $$ = new AsgBinaryNode($1, node);
                        pool.add(node);
                        pool.add($$);
		}
		}
	| testlist star_EQUAL { if($2 != NULL) { $$ = new AsgBinaryNode($1, $2);
				pool.add($$);}
		}
	;
...
print_stmt // Used in: small_stmt
	: PRINT opt_test { 
			if( TableManager::getInstance().getScope() == 0 ){
			$$ = new PrintNode($2); 
			$$->eval();}
			else{ $$ = new PrintNode($2);}
				}
...
flow_stmt // Used in: small_stmt
	: break_stmt
	| continue_stmt
	| return_stmt { if($1 == NULL){	$$ = new ReturnNode(NULL); pool.add($$); }
			else { $$= new ReturnNode($1); pool.add($$); } }
	| raise_stmt
	| yield_stmt
	;
...
suite // Used in: funcdef, if_stmt, star_ELIF, while_stmt, for_stmt, try_stmt, plus_except, opt_ELSE, opt_FINALLY, with_stmt, classdef
	: simple_stmt  
	| NEWLINE INDENT plus_stmt DEDENT { $$ = new SuiteNode(*$3);}
	;
...
comparison // Used in: not_test, comparison
	...
	| comparison comp_op expr {
			switch($2) {
				case 1:{ $$ = new LessBinaryNode($1,$3); pool.add($$); break;}
				case 2:{ $$ = new GreaterBinaryNode($1,$3); pool.add($$); break;}
				case 3:{ $$ = new EqEqBinaryNode($1,$3); pool.add($$); break;}
				case 7:{ $$ = new NotEqBinaryNode($1,$3); pool.add($$); break;}
				case 4:{ $$ = new GreaterEqBinaryNode($1,$3); pool.add($$); break;}
				case 5:{ $$ = new LessEqBinaryNode($1,$3); pool.add($$); break;}
			}
		}
	;
...
arith_expr // Used in: shift_expr, arith_expr
	: term
	| arith_expr pick_PLUS_MINUS term { if ($2 == "PLUS") $$ = new AddBinaryNode($1, $3);
		if($2 == "MINUS") $$ = new SubBinaryNode($1,$3);
		pool.add($$);
		}
	;
...
term // Used in: arith_expr, term
	: factor
	| term pick_multop factor { if($2 == "MUL"){ $$ = new MulBinaryNode($1,$3);
				pool.add($$);
				}
				if($2 == "DIV") {$$ = new DivBinaryNode($1,$3);
				pool.add($$);
				}
				if($2 == "PERCENT") {
	...
	;
pick_multop // Used in: term   -----------------------------Binary Ops * / % //
	: STAR  { $$ = "MUL";}
	| SLASH { $$ = "DIV";}
	| PERCENT { $$ = "PERCENT";}
	| DOUBLESLASH { $$ = "DIV";}
...
factor // Used in: term, factor, power
	: pick_unop factor {	if($1 == "POS") {$$ = new PosUnaryNode($2);
				pool.add($$);
				}
				if($1 == "NEG") {$$ = new NegUnaryNode($2);
				pool.add($$);}
			   } 
	| power
	;
...
power // Used in: factor
	: atom star_trailer DOUBLESTAR factor { $$ = new PowBinaryNode($1,$4);
						pool.add($$);}
	| atom star_trailer { if (($2==1) &&($1 != NULL)){
			Node* cn = new CallNode(static_cast<IdentNode*>($1)->getIdent());
			if( TableManager::getInstance().getScope() == 0){
				const Node* n = cn->eval();
				if(n != NULL){
					$$ = const_cast<Node*>(n);}
			}
			else { $$ = cn;}
...
star_trailer // Used in: power, star_trailer
	...
	| %empty
	;
atom // Used in: power ------------------------------NUMBERS / IDENTS
